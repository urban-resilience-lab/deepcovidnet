...
def add_hyperparameters(hps):
    with hps(level=HPLevel.HIGH):
        hps.add(name='lr', val=0.00050828, hp_range=[0.000001, 1], log_scale=True)
        hps.add(name='batch_size', val=128, hp_range=[16, 256], hp_type=int)
        hps.add(name='embedding_size', val=283, hp_range=[32, 512], hp_type=int)
        hps.add(name='higher_order_features_size', val=1424, hp_range=[32, 2048], hp_type=int)
        hps.add(name='deep_intermediate_size', val=512, hp_range=[64, 1024], hp_type=int)
        hps.add(name='deep_layers', val=4, hp_range=[2, 5], hp_type=int)
        hps.add(name='dropout_prob', val=0.1, hp_range=[0, 1])

    with hps(level=HPLevel.MEDIUM):
        pass

    with hps(level=HPLevel.LOW):
        hps.add(name='weight_decay', val=0.0001, hp_range=[0.000001, 0.01], log_scale=True)
